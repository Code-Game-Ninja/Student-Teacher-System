...
export default function TeacherHistory() {
  ...
  const fetchAppointments = async () => {
    if (!teacherId) return;
    setFetching(true);
    const q = query(collection(db, "appointments"), where("teacherId", "==", teacherId), where("status", "!=", "pending"));
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-gradient-to-br from-blue-100 via-purple-100 to-green-100 p-8">
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-2xl bg-white bg-opacity-90 rounded-3xl shadow-2xl p-8 mt-10 text-center">
        <h2 className="text-2xl font-bold text-blue-700 mb-6">Appointment History</h2>
        {fetching ? (
          <div className="flex justify-center py-10"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500"></div></div>
        ) : appointments.length === 0 ? (
          <div className="text-center text-gray-500 py-10">No past appointments found.</div>
        ) : (
